assemble_structure <- function(flamingo_backbone_prediction_obj,flamingo_final_res_data_obj,list_of_flamingo_domain_prediction_obj, max_iter=500){
  ...
  input_if = as.matrix(flamingo_final_res_data_obj@IF)
  input_if <- input_if + t(input_if)
  diag(input_if) <- diag(input_if)/2
  pd <- if2pd(input_if, -0.25, 3)
  error <- 10
